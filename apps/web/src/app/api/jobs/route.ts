...
async function proxy(request: Request, body?: unknown) {
  ...
  try {
    ...
    const query = body
      ? ""
      : `?projectId=${projectIdSchema.parse(new URL(request.url).searchParams.get("projectId"))}`;
    const response = await fetch(`http://127.0.0.1:${port}/jobs${query}`, {
...
export async function GET(request: Request) {
  if (
    ...
    return Response.json({ error: "Geçersiz proje kimliği." }, { status: 400 });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const input = generationRequestSchema.safeParse(JSON.parse(text));
    if (!input.success)
      return Response.json(
        {
          error:
            "Proje bilgileri geçersiz. Onayları ve zorunlu alanları kontrol edin.",
        },
        { status: 400 },
      );
    return proxy(request, input.data);
  } catch {
