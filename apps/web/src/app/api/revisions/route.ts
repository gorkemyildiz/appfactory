...
import { revisionRequestSchema, projectIdSchema } from "@app-factory/schemas";
...
async function proxy(request: Request, body?: unknown) {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${port}/revisions${query}`, {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return proxy(request, revisionRequestSchema.parse(body));
  } catch {
    return Response.json(
